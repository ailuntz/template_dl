...
## 增量更新说明
新增任务时，按下面顺序补齐注册与配置即可：
- 注册任务模块：`src/lit_modules/__init__.py`
- 注册模型：`src/models/__init__.py`
- 注册层（如有）：`src/layers/__init__.py`
- 注册预处理：`src/preprocess/__init__.py`
- 注册预测输出：`src/predict/__init__.py`
- 注册数据集：`src/data/__init__.py`
- 新增配置：`configs/custom/<task>.yaml`
